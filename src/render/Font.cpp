...
void pmlike::render::Font::renderText(const char *text, int x, int y, int size, int lineSpacing, glm::mat4 projection, glm::vec4 color) {
    renderText(text, x, y, -1, -1, size, lineSpacing, projection, color);
}

void pmlike::render::Font::renderText(const char *text, int x, int y, int size, int lineSpacing, glm::mat4 projection, glm::vec4 color, Alignment horizontal, Alignment vertical) {
...
void pmlike::render::Font::renderText(const char *text, int x, int y, int width, int height, int size, int lineSpacing, glm::mat4 projection, glm::vec4 color) {
    std::vector<uint32_t> strChars = convertText(text);
    renderText(strChars, x, y, width, height, size, lineSpacing,  projection, color, BEGIN, BEGIN);
}

void pmlike::render::Font::renderText(const char *text, int x, int y, int width, int height, int size, int lineSpacing, glm::mat4 projection, glm::vec4 color, Alignment horizontal, Alignment vertical) {
    std::vector<uint32_t> strChars = convertText(text);
    renderText(strChars, x, y, width, height, size, lineSpacing,  projection, color, horizontal, vertical);
}

void pmlike::render::Font::renderText(const char32_t* text, int x, int y, int width, int height, int size, int lineSpacing, glm::mat4 projection, glm::vec4 color) {
    std::vector<uint32_t> strChars = convertText((char*) text);
    renderText(strChars, x, y, width, height, size, lineSpacing,  projection, color, BEGIN, BEGIN);
}

void pmlike::render::Font::renderText(const char32_t *text, int x, int y, int size, int lineSpacing, glm::mat4 projection, glm::vec4 color) {
    std::vector<uint32_t> strChars = convertText((char*) text);
    renderText(strChars, x, y, -1, -1, size, lineSpacing, projection, color, BEGIN, BEGIN);
}


void pmlike::render::Font::renderText(std::vector<uint32_t> strChars, int x, int y, int width, int height, int size, int lineSpacing, glm::mat4 projection, glm::vec4 color, Alignment horizontal, Alignment vertical) {
    ...
    for(int i = 0; i < strChars.size(); i ++) {
        ...
        if(i < strChars.size() - 1) {
            ...
            if(this->kernings[charCode].find(nextCharCode) != this->kernings[charCode].end()) {
                currentLineWidth += blockChar.xadvance * sizingFactor + this->kernings[charCode][nextCharCode] * sizingFactor;
            } else {
    ...
    int textHeight = lineWidth.size() * (this->commonInfo.lineHeight * sizingFactor) + (std::max(lineWidth.size() - 1, (size_t)0)) * lineSpacing;
    ...
    for(int lineNr = 0; lineNr < lineWidth.size(); lineNr++) {
        ...
        for(int64_t i = lineBreaks[lineNr]; i < strChars.size(); i ++) {
            ...
            verticesPerPage[blockChar.page].insert(verticesPerPage[blockChar.page].end(), vertices.begin(), vertices.end());
    ...
    for(auto it = verticesPerPage.begin(); it != verticesPerPage.end(); it++) {
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, indicesPerPage[it->first].size() * sizeof(GLuint), indicesPerPage[it->first].data(), GL_DYNAMIC_DRAW);
