...
void World::chunkWorkerLoad(World *world) {
    ...
    while (true) {
        ...
        glm::ivec3 cameraChunkCoordinates = (world->currentCamera->position /
        ...
        for (int v = 0; v < 1; v++) {
            ...
            for (int r = 0; r < world->renderDistance; r++) {
                for (int x = -r; x <= r; x++) {
                    glm::ivec3 chunkCoords = {cameraChunkCoordinates.x + x, cameraChunkCoordinates.y + y,
                                              cameraChunkCoordinates.z + r};
                    loadChunk(world, chunkCoords);
                }
                for (int z = r - 1; z >= -r; z--) {
                    glm::ivec3 chunkCoords = {cameraChunkCoordinates.x + r, cameraChunkCoordinates.y + y,
                                              cameraChunkCoordinates.z + z};
                    loadChunk(world, chunkCoords);
                }
                for (int x = r - 1; x >= -r; x--) {
                    glm::ivec3 chunkCoords = {cameraChunkCoordinates.x + x, cameraChunkCoordinates.y + y,
...
void World::chunkWorkerGenerate(World *world) {
    while (true) {
        ...
        glm::vec3 chunkMinP = glm::vec3(chunkCoordinates.x * CHUNK_SIZE_X, chunkCoordinates.y * CHUNK_SIZE_Y,
...
void World::queueChunkGeneration(World *world, glm::ivec3 chunkCoordinates) {
    if (std::find(world->queueChunkGenerate.begin(), world->queueChunkGenerate.end(), chunkCoordinates) !=
        world->queueChunkGenerate.end()) {
        return;
    }
    ...
    std::sort(world->queueChunkGenerate.begin(), world->queueChunkGenerate.end(), [world](glm::ivec3 a, glm::ivec3 b) {
        glm::vec3 aPos = glm::vec3(a.x * CHUNK_SIZE_X, a.y * CHUNK_SIZE_Y, a.z * CHUNK_SIZE_Z) +
                         glm::vec3(CHUNK_SIZE_X / 2, CHUNK_SIZE_Y / 2, CHUNK_SIZE_Z / 2);
        glm::vec3 bPos = glm::vec3(b.x * CHUNK_SIZE_X, b.y * CHUNK_SIZE_Y, b.z * CHUNK_SIZE_Z) +
                         glm::vec3(CHUNK_SIZE_X / 2, CHUNK_SIZE_Y / 2, CHUNK_SIZE_Z / 2);
